...
def call_argus_for_label(json_payload):
    url = 'http://10.66.29.42:8087/argus/get/userlabels'
    header = {'authorization': 'P0s6jZGIkwMrnS2',
              'accept': 'application/json',
              'content-type': 'application/json',
              'x-request-tracker': 'testIndc1'}
    req_payload = {"locations": json_payload}
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                print("COmpressed result set ", rs)
                ...
                gmap.scatter(rs['latitude'], rs['longitude'], '#FF0000', size=20, marker=True)
...
def plot_graph(request):
    ...
    rs = pd.DataFrame.from_dict(r_json, orient='columns')

    #json_file = "locations/rs.json"
    #rs = pd.read_json(json_file)
...
def gmap_details(request):
    ...
    rs_old = pd.DataFrame.from_dict(data_json, orient='columns')
    rs = rs_old.head(10)
    # Google maps integration
    for index, row in rs.iterrows():
        googlemps = googlemaps.Client(key='AIzaSyBVWUMz35OVBSu8jQrkXGXpFu2z_R7fIJU')
    ...
    label_response_argus = call_argus_for_label(r_json1)
    ...
    data = {
        ...
        "message": msg,
        "silcoeff": silcoeff,
        "location_tag": json.dumps(label_response_argus),
